...
$(document).ready(function(){
  ...
  $("#adds").bind("click", function(){
    var selects=$("#typeIds").val();
	  var url="<c:url value='/info/area/edit.jhtml'/>?vo.parentArea.id="+selects;
  ...
  $("#updates").bind("click", function(){
	...
	if(selects == "" || selects == null){
	  alert("请选择指定的地区进行编辑操作");
	}else{
	  var url="<c:url value='/info/area/edit.jhtml'/>?id="+selects+"&key="+key;
	  window.location.href=url;
	}
  });
  ...
  $("#delete").bind("click", function(){
	...
	if(selects == "" || selects == null){
	  ...
	  if(confirm("确认删除该地区吗？")){
		var url="<c:url value='/info/area/delete.jhtml'/>?id="+selects+"&keys="+key;
	    window.location.href =url;
	  }
	}
  ...
  $("#imp").bind("click", function(){
    var url="<c:url value='/info/area/imp.jhtml'/>";
	window.location.href =url;  
  });
...
//导出按钮事件
	 $("#expBtn").click(function(){
		...
		var url="<c:url value='/info/area/exportArea.jhtml'/>";
		url+="?vo.areaName=" + areaName;
...
<body>
	<div class="jt-container">
		<div class="ifrm-cont">
			<div class="pos clear">
				<h2 class="title01-cont floatL">地区管理</h2>
			</div>
			<div>
