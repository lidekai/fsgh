...
$(document).ready(function(){
	...
	$("#expBtn").click(function(){
		...
		var month=$("#month").val();
		var url="<c:url value='/info/store-product-stock/export.jhtml'/>";
		url+="?vo.storeProduct.store.custom.customName=" + customName + "&vo.storeProduct.store.storeName="+ storeName
				...
				+ "&vo.storeProduct.product.stockCde=" + stockCde+ "&vo.month=" + month;
     ...
     <input type="hidden" name="keys" value="" id="keys"/>
      ...
      <ul class="row-fluid"> 
      	<div class="span10">
 	        ...
 	        <li>
 	        	<label>年月：</label>
 	        	<input class="input-medium Wdate" id="month" name="vo.month" value="${vo.month}" type="text" readonly="readonly"	onfocus="WdatePicker({dateFmt:'yyyy-MM'})"/>
 	        </li>
        </div>
        <div class="span2">
   ...
   <div class="span12" style="min-width:1000px;">
         <div class="pull-right  mB10">
       	...
       	<display:table name="pageList" id="vo" class="table table-bordered table-striped table-hover">
	        ...
	        <display:column title="存货编码" style="white-space:nowrap;min-width:80px">${vo.storeProduct.product.stockCde}</display:column>
	        <display:column title="产品货号" style="white-space:nowrap;min-width:80px">${vo.storeProduct.product.number}</display:column>
	        ...
	        <display:column title="数量" style="white-space:nowrap;min-width:20px">${vo.count}</display:column>
	        <display:column title="金额" style="white-space:nowrap;min-width:50px">${vo.money}</display:column>
	        <display:column title="年月" style="white-space:nowrap;min-width:50px">${vo.month}</display:column>
            <display:column title="操  作" style="min-width:150px">
            	...
            	<pt:checkFunc code="INFO_STOPROSTO_EDIT">
               		<button name="edit" class="btn btn-small btn-success" att-id="${vo.id }" att-key="${vo.key }"><i class="icon-edit"></i>编辑</button>
               	</pt:checkFunc>
            </display:column>
            <display:setProperty name="paging.banner.item_name">门店库存</display:setProperty>
            <display:setProperty name="paging.banner.items_name">门店库存</display:setProperty>
	    </display:table>
