...
$(document).ready(function(){
  ...
  $("#adds").bind("click", function(){
    var selects=$("#typeIds").val();
	  var url="<c:url value='/info/customs-type/edit.jhtml'/>?vo.parents.id="+selects;
  ...
  $("#updates").bind("click", function(){
	...
	if(selects == "" || selects == null){
	  alert("请选择指定的客户类别进行编辑操作");
	}else{
	  var url="<c:url value='/info/customs-type/edit.jhtml'/>?id="+selects+"&key="+key;
	  window.location.href=url;
	}
  });
  ...
  $("#delete").bind("click", function(){
	...
	if(selects == "" || selects == null){
	  ...
	  if(confirm("确认删除该产品分类吗？")){
		var url="<c:url value='/info/customs-type/delete.jhtml'/>?id="+selects+"&keys="+key;
	    window.location.href =url;
	  }
	}
  ...
  $("#imp").bind("click", function(){
    var url="<c:url value='/info/customs-type/imp.jhtml'/>";
	window.location.href =url;  
  });
  
//导出按钮事件
	 $("#expBtn").click(function(){
		var customTypeName=$("#searchName").val();	
		var url="<c:url value='/info/customs-type/exportCustomsType.jhtml'/>";
		url+="?vo.customTypeName=" + customTypeName;
...
<body>
	<div class="jt-container">
		<div class="ifrm-cont">
			<div class="pos clear">
				<h2 class="title01-cont floatL">客户分类管理</h2>
			</div>
			<div>
				<input type="hidden" value="1" id="flag" /> <input type="hidden"
					id="parentIds" /> <input type="hidden" id="typeIds" /> <input
					type="hidden" id="key" /> <input type="hidden" id="prodsType" /> <input
					type="button" class="btn btn-small btn-success" value="收缩"
					id="expand" />
				<pt:checkFunc code="INFO_CUSTYPE_ADD">
					<input type="button" class="btn btn-small btn-success" value="添加"
						id="adds" />
				</pt:checkFunc>
				<pt:checkFunc code="INFO_CUSTYPE_EDIT">
					<input type="button" class="btn btn-small btn-success" value="编辑"
						id="updates" />
				</pt:checkFunc>
				<pt:checkFunc code="INFO_CUSTYPE_DEL">
					<input type="button" class="btn btn-small btn-success" value="删除"
						id="delete" />
				</pt:checkFunc>
				<pt:checkFunc code="INFO_CUSTYPE_IMP">
