...
$(document).ready(function(){
	...
	$("#exp").click(function(){
		...
		var url="<c:url value='/charge/out-fee-verification/exportOutFeeVer.jhtml'/>";
		url+="?vo.verCode=" + verCode +"&vo.outFeeProvision.provisionCode=" + provisionCode+"&vo.verDirection="+verDirection; 
		url+="&vo.beginTime="+beginTime+"&vo.endTime="+endTime+"&vo.verType.dictName="+dictName+"&vo.approveState="+approveState;
		url+="&vo.outFeeProvision.provisionProject.projectType="+projectType + "&vo.custom.customName=" + customName;
      ...
      <ul class="row-fluid"> 
      	<div class="span10">
 	        ...
 	        <li>
      		...
      		<li>
 	        	<label>所属预提：</label>
 	        	<input class="input-medium" id="provisionCode" name="vo.outFeeProvision.provisionCode" value="${vo.outFeeProvision.provisionCode}" type="text" placeholder="输入需要搜索的编码" />
 	        </li>
 	        <li>
 	        	<label>核销方向：</label>
 	        ...
 	        <li>
 	        	<label>项目类型：</label>
 	        	<select name="vo.outFeeProvision.provisionProject.projectType" class="span150" id="projectType">
 	        		<option value="">请选择</option>
 	        		<c:forEach items="<%=PublicType.ProjectType.values() %>" var="type">
 	        			<option value="${type.name }" <c:if test="${vo.outFeeProvision.provisionProject.projectType  == type.name}">selected</c:if>>${type.text }</option>
 	        		</c:forEach>
 	        	</select>
 	        </li>
 	        <li>
   ...
   <div class="span12" style="min-width:1400px;">
         <div class="pull-right  mB10">
       	...
       	<display:table name="pageList" id="vo" class="table table-bordered table-striped table-hover">
	        ...
	        <display:column title="核销编码" style="white-space:nowrap">${vo.verCode}</display:column>
	        <display:column title="所属预提" style="white-space:nowrap">${vo.outFeeProvision.provisionCode}</display:column>
	        ...
	        <display:column title="项目类型" style="white-space:nowrap">${vo.outFeeProvision.provisionProject.projectType.text}</display:column>
	        ...
	        <display:column title="总费用(元)" style="white-space:nowrap"><fmt:formatNumber value="${vo.totalFee}" type="currency" /></display:column>
	        <display:column title="核销时间" style="white-space:nowrap"><fmt:formatDate value="${vo.verTime}" pattern="yyyy-MM-dd" /></display:column>
	        <display:column title="审核状态" style="white-space:nowrap">${vo.approveState.text}</display:column>
            <display:column title="生成收款单" style="white-space:nowrap">${vo.isCreateU8 ? '是' : '否'}</display:column>
            <display:column title="操  作" style="width:250px;">
            	...
            	<c:if test="${vo.approveState == 'SPJS' }">
            		<pt:checkFunc code="CHAR_OUTFEE_REA">
